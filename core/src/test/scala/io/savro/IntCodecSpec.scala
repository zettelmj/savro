...
class IntCodecSpec extends AnyWordSpec with Matchers {
  ...
  "decode" must {
    ...
    "FE FF FF FF 0F will decode to max int" in {
      val bits = hex"FE FF FF FF 0F".toBitVector
      intCodec.decode(bits) mustBe successful(DecodeResult(Int.MaxValue, BitVector.empty))
    }
    "FF FF FF FF 0F will decode to min int" in {
      val bits = hex"FF FF FF FF 0F".toBitVector
  ...
  "encode" must {
    ...
    "encoding max int works" in {
      val expectedBits = hex"FE FF FF FF 0F".toBitVector
      intCodec.encode(Int.MaxValue) mustBe successful(expectedBits)
    }
    "encoding min int works" in {
      val expectedBits = hex"FF FF FF FF 0F".toBitVector
